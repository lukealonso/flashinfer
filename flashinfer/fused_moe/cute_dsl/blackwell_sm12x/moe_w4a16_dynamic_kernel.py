...
class MoEDynamicKernel:
    ...
    def __init__(
        ...
        *,
        ...
        self.load_stage_sync_barrier = pipeline.NamedBarrier(
            barrier_id=4,
            num_threads=self.num_load_warps * self.num_threads_per_warp,
        )
    ...
    def _shared_storage_size_bytes(
        ...
        offset = 8 * 4 + 3 * (self.ab_stage * 2 * 8) + 2 * self.tile_shape_mnk[0] * 4
        buffers = [
    ...
    def _stage_w13_fp4_b_tile(
        ...
        while copy_idx < total_blocks:
            ...
            scale_group_lane_offset = local_sf_block & Int32(3)
            scale_group_offset = scale_offset - Int64(scale_group_lane_offset)
            scale_word = Uint32(0)
            if scale_group_lane_offset == Int32(0):
                scale_word = ld_global_nc_u32(sf_base + scale_group_offset)
            scale_src_lane = (copy_idx & Int32(31)) - scale_group_lane_offset
            scale_word = cute.arch.shuffle_sync(scale_word, scale_src_lane)
            scale_shift = scale_group_lane_offset << Int32(3)
            scale_byte = (scale_word >> scale_shift) & Uint32(0xFF)
    ...
    def kernel(
        ...
        phase2_pipeline = pipeline.PipelineAsync.create(
            num_stages=self.ab_stage,
        ...
        a_input_u32 = cute.recast_tensor(a_input, cutlass.Uint32)
        packed_a_u32 = cute.recast_tensor(packed_a, cutlass.Uint32)
        ...
        scatter_total = num_tokens * cols_u32
        j = flat_tid
        ...
        while produce_active > Int32(0):
            ...
            if batch_base >= total_pairs:
                ...
                if pair_idx < total_pairs:
                    ...
                    col = lane_id
                    while col < cols_u32:
                        packed_a_u32[phys_row, col, Int32(0)] = a_input_u32[
                            token_idx, col
                        ]
                        col += Int32(32)
            ...
            if has_task == Int32(0):
                ...
                while slice_idx < task_slice_count_val:
                    ...
                    if self.is_gated:
                        ...
                        csB_p = csB[None, None, None, up_cons_state.index]
                        cute.copy(smem_copy_A, csA_p[None, None, 0], crA[None, None, 0])
                        cute.copy(smem_copy_B, csB_p[None, None, 0], crB[None, None, 0])
                        for _k_tile in range(0, fc1_k_tile_cnt - 1, 1, unroll=4):  # type: ignore[call-overload]
                            for k_block_idx in cutlass.range_constexpr(num_k_blocks):
                    ...
                    # The phase2 pipeline only orders manually dequantized B_down
                    # BF16 tiles; A-side BF16 registers are hoisted once.
                    # ============================================================
                ...
                load_copy_start = lane_id + load_warp_local * Int32(
                    self.num_threads_per_warp
                )
                load_copy_stride = Int32(
                    self.num_load_warps * self.num_threads_per_warp
                )
                ...
                while slice_idx < task_slice_count_val:
                    ...
                    for k_tile in range(0, fc1_k_tile_cnt, 1, unroll=4):  # type: ignore[call-overload]
                        ...
                        self.load_stage_sync_barrier.arrive_and_wait()
                    ...
                    # No barrier needed: sB is free after the gate pass and
